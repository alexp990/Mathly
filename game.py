...
class MentalMathApp:
    ...
    def view_data_avgs(self):
        ...
        # Hide UI elements before displaying the graph
        self.change_modes_button.pack_forget()
        self.question_mode_select_box.pack_forget()
        self.difficulty_frame.pack_forget()
        self.view_data_button.pack_forget()
        ...
        # Convert the "Timestamp" column to datetime (index 4)
        df[4] = pd.to_datetime(df[4])

        # Extract only the date part (ignore time) from the "Timestamp" column (index 4)
        df["Date"] = df[4].dt.date

        # Dictionary to store average times per day for each operation
        operation_avgs = {}
        ...
        for operation in operations:
            # Filter data for the specific operation (index 0 for operation)
            op_data = df[df[0] == operation]

            # Compute the average time per day for the operation (index 2 for Time Taken)
            daily_avg = op_data.groupby("Date")[2].mean()

            # Store the results in the dictionary
            operation_avgs[operation] = daily_avg

        def update_graph():
            ax.clear()

            # Plot a line for each operation
            for operation, daily_avg in operation_avgs.items():
                ax.plot(daily_avg.index, daily_avg.values,
                        marker="o", linestyle="-", label=operation)

            ax.set_title("Average Time Per Day (by Operation)")
            ax.set_xlabel("Date")
            ax.set_ylabel("Average Time (seconds)")
            ax.legend()  # Show legend for different operations
        ...
        # Set up canvas to display the graph
        canvas = FigureCanvasTkAgg(fig, master=self.root)
        self.canvas_widget = canvas.get_tk_widget()
        self.canvas_widget.pack(fill=ctk.BOTH, expand=True)

        update_graph()  # Generate the graph initially
